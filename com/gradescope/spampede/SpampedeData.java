...
class SpampedeData {
	...
	/* --------------------- */
	/**
	 * Puts headCell at the head of the snakeHead 
	 */
	public void updateSnakeHead(BoardCell headCell) {
		this.getSnakeHead().becomeBody();
		this.getSnakeCells().addLast(headCell);
		this.getSnakeHead().becomeHead();
	}
	/**
	 * removes the snake tail
	 */
	public void removeSnakeTail() {
		this.getSnakeTail().becomeOpen();
		this.getSnakeCells().removeFirst();
	 ...
	 *         If cell is a boundary or is null, we return null
	 */
	public BoardCell getNorthNeighbor(BoardCell cell) {
		int row = cell.getRow();
		int col = cell.getColumn();
		int squareToMove = row-1;
		
		BoardCell neighbor = this.boardCells2D[squareToMove][col];
		return neighbor;
	}
	...
	public BoardCell getEastNeighbor(BoardCell cell) {
		int row = cell.getRow();
		int col = cell.getColumn();
		int squareToMove = col+1;
		
		BoardCell neighbor = this.boardCells2D[row][squareToMove];
		return neighbor;
	...
	/**
	 * 
	 * @return the current mode of the snake
	 */
	public SnakeMode getCurrentMode() {
	...
	public void reverseSnakeBody() {
		
		int size = this.getSnakeCells().size();
		
		LinkedList<BoardCell> newSnakeCells = new LinkedList<BoardCell>(); 
		//Remakes the snakeCells linked list
		for(int i = 0; i < size; i++) {
			BoardCell curr = this.getSnakeCells().pop();
			newSnakeCells.addFirst(curr);
		}
	...
	public void calcReverseDirection() {
		BoardCell tail = this.getSnakeHead();
		int currX = tail.getRow();
		int currY = tail.getColumn();
		
		BoardCell last = this.getSnakeCells().removeLast();
		String secondTailString = this.getSnakeHead().toString();
		this.getSnakeCells().addLast(last);
		
		if(boardCells2D[currX-1][currY].toString().equals(secondTailString)) {
			//if the parent of this cell is in the north, make the new direction south
			this.setDirectionSouth();
		} else if(boardCells2D[currX+1][currY].toString().equals(secondTailString)) {
			//if the parent of this cell is in the south, make the new direction north
			this.setDirectionNorth();
		} else if(boardCells2D[currX][currY+1].toString().equals(secondTailString)) {
			//if the parent of this cell is in the east, make the new direction west

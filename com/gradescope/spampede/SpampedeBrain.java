...
public class SpampedeBrain extends SpampedeBrainParent {
	...
	public void advanceTheSnake(BoardCell nextCell) {
		...
		if (nextCell.isWall() || nextCell.isBody()) {
			...
			this.theData.removeSnakeTail();
		}
		this.theData.updateSnakeHead(nextCell);
	}
	...
	public BoardCell getNextCellFromBFS() {
		...
		while(!cellsToSearch.isEmpty()) {
			BoardCell checkCells = cellsToSearch.poll();
			BoardCell[] directions = 	{this.theData.getNorthNeighbor(checkCells), 
										 this.theData.getSouthNeighbor(checkCells),
										 this.theData.getEastNeighbor(checkCells),
										 this.theData.getWestNeighbor(checkCells)};
			
			for(int i = 0; i < directions.length; i++) {
				BoardCell neighborCell = directions[i];
				if(!neighborCell.inSearchListAlready() && neighborCell.isOpen()) {
					cellsToSearch.add(neighborCell);
					neighborCell.setAddedToSearchList();
					neighborCell.setParent(checkCells);
					if(neighborCell.isSpam()) {
						return this.getFirstCellInPath(neighborCell);
					}
				}
			}
	...
	private BoardCell getFirstCellInPath(BoardCell start) {
		if(start.getParent().toString().equals(this.theData.getSnakeHead().toString())) {
			return start;
		} else {
			return this.getFirstCellInPath(start.getParent());
		}
	}
	...
	public void reverseSnake() {
		...
		//make head tail
		this.theData.reverseSnakeBody();
		//reverse the list
		this.theData.relabelHead();
		//change direction
		this.theData.calcReverseDirection();
	}
